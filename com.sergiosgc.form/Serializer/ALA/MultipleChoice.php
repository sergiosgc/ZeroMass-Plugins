...
/** 
 * Serializer_ALA_MultipleChoice is an multiple-choice input serializer to xhtml that produces code according to ALA's accessible forms article
 ...
 */
class Serializer_ALA_MultipleChoice
{
    public static function serialize(Serializer_ALA $parentSerializer, Input_MultipleChoice $choice)
    {
        if (count($choice->getChoices()) > 3) {
            if (count($choice->getRestrictionsByClass('\com\sergiosgc\form\Restriction_ClosedChoice')) > 0) {
                return Serializer_ALA_DropdownList::serialize($parentSerializer, $choice);
            } else {
                return Serializer_ALA_ComboList::serialize($parentSerializer, $choice);
            }
        } else {
            if (count($choice->getRestrictionsByClass('\com\sergiosgc\form\Restriction_ExclusiveChoice')) > 0) {
                return Serializer_ALA_RadioButton::serialize($parentSerializer, $choice);
            } else {
                return Serializer_ALA_Checkbox::serialize($parentSerializer, $choice);
            }
        }
    }
}
